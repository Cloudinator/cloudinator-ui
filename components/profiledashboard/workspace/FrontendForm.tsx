    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error?.status === 'PARSING_ERROR' && error?.originalStatus === 200) {
                toast({
                    title: "Success",
                    description: error?.data?.message || `Frontend service deployment "${projectFields.name}" created successfully!`,
                    variant: "success",
                    duration: 3000,
                });
                ...
                toast({
                    title: "Error",
                    description: error?.data?.message || "Failed to create frontend service deployment. Please try again.",
                    variant: "error",
                    duration: 5000,
                });
            }
