...
export const StreamingLog = ({ name, buildNumber }: StreamingLogProps) => {
  ...
  const getLogClass = (log: string) => {
    if (log.toLowerCase().includes("error")) return "text-red-500 dark:text-red-400";
    if (log.toLowerCase().includes("warning")) return "text-yellow-500 dark:text-yellow-400";
    if (log.toLowerCase().includes("info")) return "text-blue-500 dark:text-blue-400";
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center space-x-2">
            ...
            />
            <label htmlFor="auto-scroll" className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Auto-scroll
            </label>
          </div>
          {isDeploying && (
        ...
        <div
          ref={logContainerRef}
          className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg overflow-auto h-[600px] font-mono text-sm"
        >
          {logs.map((log, index) => (
