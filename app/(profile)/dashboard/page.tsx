...
export default function DashboardPage() {
  const { data, isLoading: workspacesLoading } = useGetWorkspacesQuery();

  const { data: buildAnalytic, isLoading: buildAnalyticLoading } =
    useGetBuildAnalyticQuery();

  const { data: countWorkspace, isLoading: workspaceCountLoading } =
    useCountWorkspaceQuery();

  const { data: countSubWorkspace, isLoading: subWorkspaceLoading } =
    useCountSubworkspacesQuery({
      name: data ? data[0]?.name : "",
    });

  const { data: countServices, isLoading: servicesCountLoading } =
    useCountServiceQuery({
      name: data ? data[0]?.name : "",
    });
  ...
  const { data: servicesData, isLoading: servicesLoading } =
    useGetServiceDeploymentQuery({
      ...
      size: 10,
      page: 0,
    }) as unknown as { data: ServiceDeploymentResponse; isLoading: boolean };

  const totalBuildAnalytic =
    Number(buildAnalytic?.fail) + Number(buildAnalytic?.success);

  const successRate =
    (Number(buildAnalytic?.success) * 100) / totalBuildAnalytic;
  ...
  const isLoading =
    workspacesLoading ||
    buildAnalyticLoading ||
    workspaceCountLoading ||
    subWorkspaceLoading ||
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {buildAnalytic?.success ? buildAnalytic?.success : 0}{" "}
            </div>
          </CardContent>
        </Card>
      </div>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4">
          ...
          <CardContent className="pl-2">
            <DeploymentOverview
              success={buildAnalytic ? buildAnalytic?.success : 0}
              failure={buildAnalytic ? buildAnalytic?.fail : 0}
            />
          </CardContent>
        </Card>
